class Twitter {
    ...
    public void postTweet(int userId, int tweetId) {
        //count in decremental order to simulate time and most recent. 
    ...
    public List<Integer> getNewsFeed(int userId) {
        ...
        for(int followeeId : followMap.get(userId)) {
            if(tweetMap.containsKey(followeeId)) {
                //get most recent tweet from each followeeId and add to heap
        ...
        while(!minHeap.isEmpty() && res.size() < 10) {
            ...
            res.add(curr[1]);
            int index = curr[3];
            if(index > 0) { //if followee has more tweets, push next to minheap
